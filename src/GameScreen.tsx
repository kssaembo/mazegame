...
export default function GameScreen({practice,initialVenue,venueCount,code,roster,difficulty,goHome}:{practice:boolean;initialVenue:number;venueCount:number;code:string;roster:string[];difficulty:Difficulty;goHome:()=>void}){
  ...
  const [match,setMatch]=useState<MatchState|null>(null),[visual,setVisual]=useState<Visual|null>(null),[bump,setBump]=useState(false),[wallFlash,setWallFlash]=useState<{position:Point;direction:Direction}|null>(null),[animating,setAnimating]=useState(false),[message,setMessage]=useState('현재 차례의 플레이어가 명령을 조립하세요.'),[drawConfirm,setDrawConfirm]=useState(false),[drawAsked,setDrawAsked]=useState(false);

  useEffect(()=>{if(practice||!venue)return;void sendVenueEvent(code,{type:'connected',venue});const peer=new Peer(`memory-maze-venue-${code}-${venue}`);peer.on('connection',conn=>conn.on('data',data=>{const type=(data as {type?:string}).type;if(type==='game-ended'||type==='game-reset'){for(let index=localStorage.length-1;index>=0;index--){const key=localStorage.key(index);if(key?.startsWith('memory-maze:'))localStorage.removeItem(key)}history.replaceState(null,'',location.pathname);setPhase('closed');setMatch(null);peer.destroy()}}));return()=>peer.destroy()},[practice,code,venue]);
  ...
  const start=()=>{const next=createMatch(names,`${code}-${venue}-${Date.now()}`,code,difficulty);setMatch(next);setDrawAsked(false);setPhase('play');if(!practice)void sendVenueEvent(code,{type:'occupied',venue})};
  const acceptDraw=async()=>{if(!match)return;const next:MatchState={...match,phase:'completed',winnerId:undefined,finishReason:'teacher_decision',completedAt:Date.now(),revision:match.revision+1};setMatch(next);setDrawConfirm(false);setMessage('100턴을 넘겨 두 플레이어가 무승부로 경기를 마쳤습니다.');if(!practice){const result={...toResult(next),venueNumber:venue};const ok=await sendVenueEvent(code,{type:'result',venue,result});setMessage(ok?'무승부 결과가 교사에게 전송됐습니다.':'무승부 결과를 기기에 보관했습니다. 연결되면 다시 전송합니다.');void sendVenueEvent(code,{type:'available',venue})}};
  const run=async(commands:Command[],useDouble:boolean)=>{
    ...
    else if(next.phase==='active'&&next.turnNumber>100&&!drawAsked){setDrawAsked(true);setDrawConfirm(true)}
  };
  ...
  return <main className="game"><header><button className="logo-button"><img src={`${IMG}icon_logo_maze_mark.png`}/></button><div><span>{practice?'연습 경기':`${venue}번 경기장`} · {match.difficulty==='easy'?'쉬움':'어려움'}</span><h1><b>{match.turnNumber}턴</b> · {match.phase==='completed'?'경기 종료':`${current.name} 차례`}</h1></div>{practice&&<button className="secondary" onClick={goHome}>메인화면으로</button>}</header><div className="game-layout">
    ...
    <section className="board-area"><div className="turn-banner"><i className={current.id==='player-a'?'mint':'coral'}>{ARROW[visual?.playerId===current.id?visual.direction:current.direction]}</i><div><b>{match.phase==='completed'?(match.winnerId?`${current.name} 승리!`:'무승부'):`${current.name} 차례`}</b><span>{message}</span></div></div><MazeBoard match={match} visual={visual} bump={bump} wallFlash={wallFlash}/><p className="secret-note">보이지 않는 벽 32개가 숨어 있습니다 · 새 경기마다 미로가 바뀝니다</p></section>
    {match.phase==='completed'?<aside className={`command-panel result-card ${match.winnerId?'victory':''}`}><div className="mini-celebration">{match.winnerId&&<><img className="mini-confetti" src={`${IMG}effect_goal_confetti.png`}/><i>★</i><i>✦</i></>}<img src={`${IMG}icon_victory_trophy.png`}/></div><h2>{match.winnerId?`${current.name} 승리!`:'무승부'}</h2><p>{match.winnerId?'멋진 알고리즘으로 목적지에 도착했습니다!':'100턴 동안 멋진 대결을 펼쳤습니다. 두 플레이어 모두 2점입니다.'}</p><button className="primary" onClick={()=>{setNames(['','']);setPhase('first');setMatch(null)}}>다음 경기</button></aside>:<CommandPanel key={match.turnNumber} match={match} disabled={animating} onRun={run}/>} 
  </div>{drawConfirm&&<div className="modal"><section className="confirm draw-confirm"><h2>100턴이 넘어갔습니다</h2><p>경기를 무승부 처리하시겠습니까?<br/>무승부 처리 시 두 플레이어 모두 승점 2점을 받습니다.</p><div><button onClick={()=>setDrawConfirm(false)}>경기 계속하기</button><button className="primary" onClick={()=>void acceptDraw()}>무승부 처리</button></div></section></div>}</main>;
}
